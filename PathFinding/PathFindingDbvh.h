...
namespace PathFinding
{
	...
	protected:
		...
		PFMap<size_t, unsigned short> _objToNodesMap;
		PFArray<BvhNode> _allNodes;
		PFArray<unsigned short> _freeIndices;
		PFSet<unsigned short> _refitIndices;
	...
	public:
		...
		template<typename ObjT>
		void CreateInitialBvh(PFArray<std::pair<ObjT*,Shape>>& objects)
		{
			if( objects.size() >= 2)
			{
		...
		// VectorT is PFVector2 or PFVector3 etc. depending on Shape dimensions
		// It may be also any other custom parameter for arbitrary query
		template<typename ObjT, typename RayT, typename VectorT>
		void TestRay(const RayT& ray, PFArray<std::pair<ObjT*, VectorT>>& hitObjects)
		{
		...
		void TestGeneric(const TestShapeT& shape, PFArray<ObjT*>& hitObjects, ObjDyscriminator& acceptHit = ObjDyscriminator())
		{
	...
	protected:
		template<typename ObjT, typename RayT, typename VectorT>
		void TestRayInternal(BvhNode& node, const RayT& ray, PFArray<std::pair<ObjT*, VectorT>>& hitObjects)
		{
		...
		void TestGenericInternal(BvhNode& node, 
			const TestShapeT& shape, PFArray<ObjT*>& hitObjects, 
		...
		template<typename ObjT>
		void CreateInitialBvhInternal(PFArray<std::pair<ObjT*,Shape>>& objects)
		{
	...
	template<typename ShapeT>
	void Dbvh<ShapeT>::DebugDraw(IDebugDrawer* drawer)
	{
