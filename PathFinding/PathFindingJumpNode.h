...
namespace PathFinding
{
	...
	public:
		...
		JumpNode(const int cellCode) :
			_cellCode(cellCode), _closed(false), _ancestor(0) { }
		JumpNode(const int cellCode, const float dist) :
			_cellCode(cellCode), _closed(false), _distFromStart(dist), _ancestor(0) { }
		JumpNode(const int cellCode, const float dist, JumpNode* anc) :
	...
	//			(targetX-_cellX)*(targetX-_cellX) + (targetY-_cellY)*(targetY-_cellY));
		...
		inline JumpNode* GetAncestor() const { return _ancestor; }
		inline void SetAncestor(JumpNode* anc) { _ancestor = anc; }
		...
		{
		public:
			inline bool operator()(const JumpNode& node1, const JumpNode& node2)
			{
