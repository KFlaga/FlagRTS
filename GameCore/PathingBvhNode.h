...
namespace FlagRTS
{
	...
	public:
		...
		{
			Vector2 minCorner1 = leftShape.GetCenter() - leftShape.GetExtends() * 0.5f;
			Vector2 minCorner2 = rightShape.GetCenter() - rightShape.GetExtends() * 0.5f;
		...
		bool ExpandNodeVolume(const Shape& shapeToFit, Array<BvhNode>* allNodes, bool recursive = true) 
		{
		...
		bool RefitVolume(Array<BvhNode>* allNodes, bool recursive = true) 
		{
			_ASSERT(!IsLeaf);
		...
		void SetDepth(unsigned int depth, Array<BvhNode>* allNodes)
		{
			Depth = depth;
			if( !IsLeaf )
			{
