...
namespace FlagRTS
{
	OgreTerrainLoader::OgreTerrainLoader(
		OgreTerrainGrid* terrainGrid, TerrainInfo* terrInfo) :
	...
	void OgreTerrainLoader::LoadTerrain()
	{
		for(int x = 0; x < _terrainInfo->GetTilesX(); x++)
			for(int z = 0; z < _terrainInfo->GetTilesZ(); z++)
				DefineTerrain(x,z);

		
		_terrainGrid->PrepareTerrains();
		_terrainGrid->LoadAllTerrains();
	...
	void OgreTerrainLoader::UnloadTerrain()
	{

	}


	void OgreTerrainLoader::LoadTerrainHeightMap(const string& mapname, uint8 x, uint8 z, Ogre::Image* img)
	{
		string finalname = mapname + "_hm_" + std::to_string(x) + "_" + std::to_string(z) + ".png";
		img->load(finalname,"Maps");
	}

	void OgreTerrainLoader::DefineTerrain(int x, int y)
	{
		Ogre::Image* img = OGRE_NEW Ogre::Image();
		LoadTerrainHeightMap(_terrainInfo->GetName() , x , y , img);
		_terrainGrid->SetHeightImage(x,y,img);
	}

	void OgreTerrainLoader::InitBlendMaps(int x, int z)
	{
		// NUm of layers = num of textures
		int maxLayer = _terrainInfo->GetTileset()->GetLoadedTexturesCount();

		// Load artefix blend maps
		Ogre::Image blendMapImage[3];
		string bmBaseName = _terrainInfo->GetName() + "_bm_";
		...
		Ogre::Terrain* terrain = _terrainGrid->GetTerrain(x,z);
