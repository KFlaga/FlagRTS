...
namespace FlagRTS
{
	...
	void UniformGridPathingMap::FindTopLeftOfObstacleWithCenterPosition(UniformGridObstacle* obstacle, const Vector2& position, int* x, int* y)
	{
	...
	void UniformGridPathingMap::DrawGrid(PathingDrawer* drawer)
	{
		...
		for(dy = 0; dy < lastY; ++dy)
		{
			...
			{
				drawer->DrawLine(
					Vector2(dx * _cellSize.x, dy * _cellSize.y),
					Vector2((dx+1) * _cellSize.x, dy * _cellSize.y));
				drawer->DrawLine(
					Vector2(dx * _cellSize.x, dy * _cellSize.y),
					Vector2(dx * _cellSize.x, (dy+1) * _cellSize.y));
			}
		}
		...
		{
			drawer->DrawLine(
				Vector2(lastX * _cellSize.x, dy * _cellSize.y),
				Vector2(lastX * _cellSize.x, (dy+1) * _cellSize.y));
		}

		for(dx = 0; dx < lastX; ++dx)
		{
			drawer->DrawLine(
				Vector2(dx * _cellSize.x, lastY * _cellSize.y),
				Vector2((dx+1) * _cellSize.x, lastY * _cellSize.y));
		}
	}

	void UniformGridPathingMap::DrawObstacles(PathingDrawer* drawer, int blockedGroups)
	{
		...
		for(dy = 0; dy < _grid.GetRowCount()-1; ++dy)
		{
			...
			{
				if(_grid(dy,dx).Blocks(CollisionFilter(blockedGroups, blockedGroups)))
				{
					drawer->DrawLine(
						Vector2(dx * _cellSize.x, dy * _cellSize.y),
						Vector2((dx+1) * _cellSize.x, (dy+1) * _cellSize.y));
