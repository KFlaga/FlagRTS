...
namespace FlagRTS
{
	...
	inline bool Box3d::IsPointInside(const Vector3& point) const
	{
	...
	{
		Vector3 v = Maximum - Minimum;
		return reinterpret_cast<float*>(&v)[axis];
	}

	inline Vector3 Box3d::GetExtends() const
	{
	...
	{
		return Box3d( Vector3(std::min( Minimum.x, other.Minimum.x ), 
			std::min(Minimum.y, other.Minimum.y), std::min(Minimum.z, other.Minimum.z)),
			Vector3(std::max(Maximum.x, other.Maximum.x),
	...
	inline Vector3 Box3d::GetCenter() const
	{
	...
	inline void Box3d::DebugDraw(PathingDrawer* drawer) const
	{
		drawer->DrawBox3d(Vector3(Minimum.x, Maximum.y, Minimum.z),
